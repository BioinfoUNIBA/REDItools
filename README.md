...
<p>Example:</p>
<div class="highlight-python"><pre>python download-prepare-data-NP.py 
where “rna_editing_protocol” is the main folder in which all data will be downloaded, <REDItools folder> is the absolute path to REDItools folder and <use paths 0/1> is a flag to use (1) or not (0) a file with absolute paths to requested programs. If  <use paths 0/1> is set to 0 and above software has been correctly installed, paths to requested programs will be automatically checked.</pre>
</div>
</div>
</div>
