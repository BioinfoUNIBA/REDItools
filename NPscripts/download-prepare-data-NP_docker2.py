...
#wcmd='%s -c --retry-connrefused --tries=0 --timeout=5  ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_30/GRCh37_mapping/GRCh37.primary_assembly.genome.fa.gz' %(prg['wget'])
#ot=getData(wcmd)
ot = 0
...
#gcmd='%s -c --retry-connrefused --tries=0 --timeout=5 ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_30/GRCh37_mapping/gencode.v30lift37.annotation.gtf.gz' %(prg['wget'])
#ot=getData(gcmd)
...
#gcmd='%s -c --retry-connrefused --tries=0 --timeout=5 --no-check-certificate https://sourceforge.net/projects/rseqc/files/BED/Human_Homo_sapiens/hg19_RefSeq.bed.gz' %(prg['wget'])
#ot=getData(gcmd)
...
os.mkdir('rmsk')
os.chdir('rmsk')
#gcmd='%s -c --retry-connrefused --tries=0 --timeout=5 http://hgdownload.soe.ucsc.edu/goldenPath/hg19/database/rmsk.txt.gz' %(prg['wget'])
#ot=getData(gcmd)
...
os.mkdir('snp151')
os.chdir('snp151')
#gcmd='%s -c --retry-connrefused --tries=0 --timeout=5  http://hgdownload.soe.ucsc.edu/goldenPath/hg19/database/snp151.txt.gz' %(prg['wget'])
#ot=getData(gcmd)
...
os.mkdir('rediportal')
os.chdir('rediportal')
gcmd='%s -c --retry-connrefused --tries=0 --timeout=5  http://srv00.recas.ba.infn.it/webshare/rediportalDownload/table1_full.txt.gz' %(prg['wget'])
op=getData(gcmd) 
if op==1: sys.stderr.write('I cannot download REDIportal annotations.\n')
...
#fq1cmd='%s -c --retry-connrefused --tries=0 --timeout=5 ftp://ftp.sra.ebi.ac.uk/vol1/fastq/ERR262/ERR262997/ERR262997_1.fastq.gz' %(prg['wget'])
#fq2cmd='%s -c --retry-connrefused --tries=0 --timeout=5 ftp://ftp.sra.ebi.ac.uk/vol1/fastq/ERR262/ERR262997/ERR262997_2.fastq.gz' %(prg['wget'])
#f1=getData(fq1cmd)
#f2=getData(fq2cmd)
f1,f2 = 0,0
...
#fq1cmd='%s -c --retry-connrefused --tries=0 --timeout=5 ftp://ftp.sra.ebi.ac.uk/vol1/fastq/SRR125/008/SRR1258218/SRR1258218_1.fastq.gz' %(prg['wget'])
#fq2cmd='%s -c --retry-connrefused --tries=0 --timeout=5 ftp://ftp.sra.ebi.ac.uk/vol1/fastq/SRR125/008/SRR1258218/SRR1258218_2.fastq.gz' %(prg['wget'])
#f1=getData(fq1cmd)
#f2=getData(fq2cmd)
if f1==1: sys.stderr.write('I cannot download READ1.\n')
else:
...
for i in fqlist:
	...
	os.mkdir(base)
	os.chdir(base)
	#fq1cmd='%s -c --retry-connrefused --tries=0 --timeout=5 %s' %(prg['wget'],fq1)
	#fq2cmd='%s -c --retry-connrefused --tries=0 --timeout=5 %s' %(prg['wget'],fq2)
	#fq1_=getData(fq1cmd)
	#fq2_=getData(fq2cmd)
	fq1_,fq2_ = 0,0
	if fq1_+fq2_>0:
...
if ot7==0:
	...
	if ot+ot1+ot2+ot3+ot4+ot5+ot6==0: sys.stderr.write('REDIportal ready.\n')
	tend = time.time()
